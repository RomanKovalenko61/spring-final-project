# Spring Final Project - Hotel Booking System

Микросервисная система управления отелями и бронированиями с использованием Spring Boot 3.5.x и Spring Cloud.

## 📚 Документация

### Быстрый старт
- **[QUICK_START.md](QUICK_START.md)** - Быстрый старт за 5 минут
- **[MAKEFILE_USAGE.md](MAKEFILE_USAGE.md)** - Полное руководство по Makefile
- **[API_EXAMPLES.md](API_EXAMPLES.md)** - Примеры API запросов с curl
- **[TESTING_GUIDE.md](TESTING_GUIDE.md)** - Документация по тестированию
- **[TESTING_README.md](TESTING_README.md)** - ⚠️ **ВАЖНО: Как запускать тесты**

## 📑 Содержание

- [🏗️ Архитектура](#️-архитектура)
- [🚀 Требования](#-требования)
- [Сборка и запуск](#сборка-и-запуск)
- [Тестирование](#тестирование)
- [🔌 API Endpoints](#-api-endpoints)
- [💾 База данных](#-база-данных)
- [🔐 Безопасность](#-безопасность)
- [⚠️ Обработка ошибок](#️-обработка-ошибок)
- [Swagger UI и документация](#-swagger-ui-и-документация)
- [Особенности реализации](#особенности-реализации)
- [Технологический стек](#технологический-стек)


## 🏗️ Архитектура

Система состоит из следующих компонентов:

```
┌─────────────────────────────────────────┐
│         API Gateway (8080)              │
│    Spring Cloud Gateway                 │
└───────────┬─────────────────────────────┘
            │
            ├──────────────┬───────────────┐
            ▼              ▼               ▼
    ┌───────────┐  ┌─────────────┐  ┌──────────────┐
    │  Eureka   │  │   Hotel     │  │   Booking    │
    │  (8761)   │  │  Service    │  │   Service    │
    │           │  │   (8081)    │  │    (8082)    │
    └───────────┘  └─────────────┘  └──────────────┘
         │               │                   │
         │          ┌────┴────┐         ┌────┴────┐
         │          │ H2 DB   │         │ H2 DB   │
         │          │ hoteldb │         │bookingdb│
         │          └─────────┘         └─────────┘
         │
    Service Discovery
```

- **Eureka Server** (порт 8761) - Service Discovery
- **API Gateway** (порт 8080) - Маршрутизация запросов, передача JWT
- **Hotel Service** (порт 8081) - Управление отелями и номерами, H2 in-memory
- **Booking Service** (порт 8082) - Бронирования, пользователи, JWT auth, H2 in-memory

## 🚀 Требования

- Java 17+
- Maven 3.6+
- Make (опционально, для удобства)

## Сборка и запуск

### Быстрый старт

```bash
# Показать все доступные команды
make help

# Собрать проект
make build

# Запустить все сервисы
make start

# Проверить статус сервисов
make status

# Остановить все сервисы
make stop
```

### Доступные Make команды

```bash
make build          # Собрать весь проект
make clean          # Очистить проект
make start          # Запустить все сервисы (проверяет порты)
make stop           # Остановить все сервисы
make restart        # Перезапустить все сервисы
make status         # Показать статус сервисов
make logs           # Показать логи всех сервисов
make logs-eureka    # Логи только Eureka
make logs-hotel     # Логи только Hotel Service
make logs-booking   # Логи только Booking Service
make logs-gateway   # Логи только Gateway
make test           # Запустить тесты
make package        # Создать jar файлы
make run-jars       # Запустить из jar файлов
make info           # Показать информацию о проекте
make api-test       # Быстрый API тест
```

### Запуск отдельного сервиса для разработки

```bash
# Запустить только один сервис
make dev SERVICE=hotel
make dev SERVICE=booking
make dev SERVICE=eureka
make dev SERVICE=gateway
```

### Альтернативный способ: Ручной запуск

```bash
# 1. Запустить Eureka Server
cd spring-final-eureka
mvn spring-boot:run

# 2. Запустить Hotel Service
cd spring-final-hotel
mvn spring-boot:run

# 3. Запустить Booking Service
cd spring-final-booking
mvn spring-boot:run

# 4. Запустить API Gateway
cd spring-final-gateway
mvn spring-boot:run
```

## Тестирование

Проект включает полный набор интеграционных тестов с покрытием всех критических сценариев.

### ⚠️ Важно для запуска тестов

**Интеграционные тесты требуют запущенных сервисов!**

```bash
# 1. Сначала запустите все сервисы
make start

# 2. Дождитесь их готовности (30-60 секунд)
make status

# 3. Запустите тесты
make test
```

### Запуск тестов

```bash
# Запустить все тесты
make test

# Или через Maven
mvn test

# Запуск тестов конкретного модуля
cd spring-final-booking
mvn test

cd spring-final-hotel
mvn test
```

### Покрытие тестами

#### Booking Service
- ✅ Аутентификация и регистрация пользователей
- ✅ Валидация входных данных
- ✅ Параллельные бронирования (race conditions)
- ✅ Идемпотентность операций
- ✅ Saga Pattern и компенсация ошибок
- ✅ Автоподбор комнат с балансировкой загрузки
- ✅ Отмена бронирований

#### Hotel Service
- ✅ CRUD операции с отелями и номерами
- ✅ Получение доступных номеров
- ✅ Рекомендации номеров (сортировка по загруженности)
- ✅ Подтверждение и освобождение резерваций
- ✅ Обнаружение конфликтов при двойном бронировании

Подробнее см. **[TESTING_GUIDE.md](TESTING_GUIDE.md)**



## Тестовые пользователи

После запуска Booking Service автоматически создаются тестовые пользователи:

- **Admin**: username=`admin`, password=`admin123`
- **User**: username=`user`, password=`user123`

## Тестовые данные

Hotel Service автоматически создает:
- 2 отеля (Grand Hotel, Business Hotel)
- 10 номеров различных типов

## 🔌 API Endpoints

### Общая информация
Все запросы проходят через **API Gateway** на порту **8080**.
Большинство эндпойнтов требуют JWT токен в заголовке `Authorization: Bearer {token}`.

### Структура маршрутизации

| Путь | Сервис | Порт напрямую |
|------|--------|---------------|
| `/api/user/**` | Booking Service | 8082 |
| `/api/bookings/**` | Booking Service | 8082 |
| `/api/hotels/**` | Hotel Service | 8081 |
| `/api/rooms/**` | Hotel Service | 8081 |

### Аутентификация (через Gateway или напрямую Booking Service)

```bash
# Регистрация
POST http://localhost:8080/api/user/register
Content-Type: application/json

{
  "username": "newuser",
  "password": "password123"
}

# Аутентификация
POST http://localhost:8080/api/user/auth
Content-Type: application/json

{
  "username": "admin",
  "password": "admin123"
}
```

### Управление пользователями (ADMIN)

```bash
# Создать пользователя (ADMIN)
POST http://localhost:8080/api/user
Authorization: Bearer {admin_token}
Content-Type: application/json

{
  "username": "newuser",
  "password": "password123",
  "roles": ["ROLE_USER"]
}

# Обновить пользователя (ADMIN)
PATCH http://localhost:8080/api/user/{id}
Authorization: Bearer {admin_token}
Content-Type: application/json

{
  "username": "updated_username",
  "enabled": true
}

# Удалить пользователя (ADMIN)
DELETE http://localhost:8080/api/user/{id}
Authorization: Bearer {admin_token}
```

### Отели (требуется JWT токен)

```bash
# Получить список отелей
GET http://localhost:8080/api/hotels
Authorization: Bearer {token}

# Создать отель (ADMIN)
POST http://localhost:8080/api/hotels
Authorization: Bearer {token}
Content-Type: application/json

{
  "name": "New Hotel",
  "address": "789 New Street, Moscow"
}
```

### Номера

```bash
# Получить доступные номера на даты
GET http://localhost:8080/api/rooms?startDate=2026-02-01&endDate=2026-02-05
Authorization: Bearer {token}

# Получить рекомендованные номера (отсортированные по загруженности)
GET http://localhost:8080/api/rooms/recommend?startDate=2026-02-01&endDate=2026-02-05&roomType=DOUBLE
Authorization: Bearer {token}

# Фильтрация по отелю и типу номера
GET http://localhost:8080/api/rooms/recommend?hotelId=1&startDate=2026-02-01&endDate=2026-02-05&roomType=SINGLE
Authorization: Bearer {token}

# Создать номер (ADMIN)
POST http://localhost:8080/api/rooms
Authorization: Bearer {token}
Content-Type: application/json

{
  "hotelId": 1,
  "roomNumber": "303",
  "type": "SUITE",
  "pricePerNight": 15000,
  "available": true
}

# Обновить номер (ADMIN)
PUT http://localhost:8080/api/rooms/{id}
Authorization: Bearer {token}
Content-Type: application/json

{
  "roomNumber": "303A",
  "type": "SUITE",
  "pricePerNight": 18000,
  "available": true
}

# Удалить номер (ADMIN)
DELETE http://localhost:8080/api/rooms/{id}
Authorization: Bearer {token}
```

### Внутренние эндпойнты Hotel Service (не доступны через Gateway)

```bash
# Подтвердить доступность номера (используется Booking Service)
POST http://localhost:8081/api/rooms/{id}/confirm-availability
Content-Type: application/json

{
  "requestId": "unique-request-id",
  "bookingId": 123,
  "startDate": "2026-02-01",
  "endDate": "2026-02-05"
}

# Освободить резервацию (компенсация)
POST http://localhost:8081/api/rooms/{id}/release?requestId=unique-request-id
```

### Бронирования

```bash
# Создать бронирование с автоподбором комнаты
POST http://localhost:8080/api/bookings
Authorization: Bearer {token}
Content-Type: application/json

{
  "startDate": "2026-02-01",
  "endDate": "2026-02-05",
  "autoSelect": true,
  "roomType": "DOUBLE"
}

# Создать бронирование с выбором конкретной комнаты
POST http://localhost:8080/api/bookings
Authorization: Bearer {token}
Content-Type: application/json

{
  "roomId": 1,
  "startDate": "2026-02-01",
  "endDate": "2026-02-05",
  "autoSelect": false
}

# Получить историю бронирований
GET http://localhost:8080/api/bookings
Authorization: Bearer {token}

# Получить бронирование по ID
GET http://localhost:8080/api/bookings/{id}
Authorization: Bearer {token}

# Отменить бронирование
DELETE http://localhost:8080/api/bookings/{id}
Authorization: Bearer {token}
```

### Полный список эндпойнтов

#### Booking Service (через Gateway на порту 8080)

| Метод | Путь | Роль | Описание |
|-------|------|------|----------|
| POST | `/api/user/register` | PUBLIC | Регистрация нового пользователя |
| POST | `/api/user/auth` | PUBLIC | Аутентификация и получение JWT |
| POST | `/api/user` | ADMIN | Создание пользователя |
| PATCH | `/api/user/{id}` | ADMIN | Обновление пользователя |
| DELETE | `/api/user/{id}` | ADMIN | Удаление пользователя |
| POST | `/api/bookings` | USER | Создание бронирования |
| GET | `/api/bookings` | USER | Список бронирований пользователя |
| GET | `/api/bookings/{id}` | USER | Получение бронирования по ID |
| DELETE | `/api/bookings/{id}` | USER | Отмена бронирования |

#### Hotel Service (через Gateway на порту 8080)

| Метод | Путь | Роль | Описание |
|-------|------|------|----------|
| GET | `/api/hotels` | USER | Список всех отелей |
| POST | `/api/hotels` | ADMIN | Создание отеля |
| PUT | `/api/hotels/{id}` | ADMIN | Обновление отеля |
| DELETE | `/api/hotels/{id}` | ADMIN | Удаление отеля |
| GET | `/api/rooms` | USER | Доступные номера на даты |
| GET | `/api/rooms/recommend` | USER | Рекомендованные номера (сортировка по загруженности) |
| POST | `/api/rooms` | ADMIN | Создание номера |
| PUT | `/api/rooms/{id}` | ADMIN | Обновление номера |
| DELETE | `/api/rooms/{id}` | ADMIN | Удаление номера |

#### Внутренние эндпойнты (не через Gateway)

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/api/rooms/{id}/confirm-availability` | Подтверждение доступности (Saga) |
| POST | `/api/rooms/{id}/release` | Освобождение резервации (компенсация) |

## 💾 База данных

Каждый сервис использует свою in-memory H2 базу данных.

### Booking Service Database (`bookingdb`)

#### Таблица `users`
| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT | Primary Key, Auto Increment |
| username | VARCHAR(255) | Уникальное имя пользователя |
| password | VARCHAR(255) | Хэш пароля (BCrypt) |
| enabled | BOOLEAN | Активен ли пользователь |
| created_at | TIMESTAMP | Дата создания |

#### Таблица `user_roles`
| Поле | Тип | Описание |
|------|-----|----------|
| user_id | BIGINT | Foreign Key → users.id |
| role | VARCHAR(50) | Роль (ROLE_USER, ROLE_ADMIN) |

#### Таблица `bookings`
| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT | Primary Key, Auto Increment |
| request_id | VARCHAR(255) | Уникальный ID запроса (идемпотентность) |
| user_id | BIGINT | ID пользователя |
| hotel_id | BIGINT | ID отеля (может быть NULL при автоподборе) |
| room_id | BIGINT | ID номера (может быть NULL) |
| start_date | DATE | Дата заезда |
| end_date | DATE | Дата выезда |
| status | VARCHAR(20) | PENDING, CONFIRMED, CANCELLED, COMPENSATED |
| created_at | TIMESTAMP | Дата создания |
| expires_at | TIMESTAMP | Время истечения (для PENDING) |
| compensation_reason | VARCHAR(500) | Причина компенсации |
| version | INTEGER | Optimistic locking |

**Индексы:**
- UNIQUE на `request_id`
- INDEX на `user_id`, `room_id`, `status`

### Hotel Service Database (`hoteldb`)

#### Таблица `hotels`
| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT | Primary Key, Auto Increment |
| name | VARCHAR(255) | Название отеля |
| address | VARCHAR(500) | Адрес |
| created_at | TIMESTAMP | Дата создания |
| updated_at | TIMESTAMP | Дата обновления |

#### Таблица `rooms`
| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT | Primary Key, Auto Increment |
| hotel_id | BIGINT | Foreign Key → hotels.id |
| room_number | VARCHAR(50) | Номер комнаты |
| type | VARCHAR(20) | SINGLE, DOUBLE, SUITE |
| price_per_night | DECIMAL(10,2) | Цена за ночь |
| times_booked | INTEGER | Счетчик бронирований (для балансировки) |
| available | BOOLEAN | Доступна ли комната (операционная доступность) |
| version | INTEGER | Optimistic locking |

**Индексы:**
- INDEX на `hotel_id`, `room_number`, `times_booked`

#### Таблица `room_reservations`
| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT | Primary Key, Auto Increment |
| room_id | BIGINT | Foreign Key → rooms.id |
| booking_id | BIGINT | ID бронирования из Booking Service |
| request_id | VARCHAR(255) | Уникальный ID запроса (идемпотентность) |
| start_date | DATE | Дата начала резервации |
| end_date | DATE | Дата окончания резервации |
| expires_at | TIMESTAMP | Время истечения временной резервации |
| created_at | TIMESTAMP | Дата создания |

**Индексы:**
- UNIQUE на `request_id`
- INDEX на `room_id`, `booking_id`, `expires_at`

### H2 Console

- **Hotel Service:** http://localhost:8081/h2-console
  - JDBC URL: `jdbc:h2:mem:hoteldb`
  - Username: `sa`
  - Password: (пусто)

- **Booking Service:** http://localhost:8082/h2-console
  - JDBC URL: `jdbc:h2:mem:bookingdb`
  - Username: `sa`
  - Password: (пусто)

## 🔐 Безопасность

### JWT аутентификация

Система использует JWT (JSON Web Tokens) для аутентификации и авторизации.

#### Параметры JWT
- **Алгоритм:** HS384
- **Срок действия:** 1 час (3600000 ms)
- **Secret key:** Конфигурируется в `application.yml` (`jwt.secret`)

#### Структура токена
```json
{
  "sub": "username",
  "roles": ["ROLE_USER", "ROLE_ADMIN"],
  "userId": 1,
  "iat": 1704638400,
  "exp": 1704642000
}
```

#### Получение токена
```bash
# Регистрация нового пользователя
POST /api/user/register
{
  "username": "user",
  "password": "password123"
}
# Ответ: { "token": "eyJhbGc...", "username": "user" }

# Аутентификация существующего пользователя
POST /api/user/auth
{
  "username": "admin",
  "password": "admin123"
}
# Ответ: { "token": "eyJhbGc...", "username": "admin" }
```

#### Использование токена
```bash
curl -H "Authorization: Bearer eyJhbGciOiJIUzM4NCJ9..." \
  http://localhost:8080/api/bookings
```

### Роли и права доступа

#### ROLE_USER
- ✅ Просмотр отелей и номеров
- ✅ Создание собственных бронирований
- ✅ Просмотр своих бронирований
- ✅ Отмена своих бронирований
- ❌ Управление отелями/номерами
- ❌ Управление пользователями

#### ROLE_ADMIN
- ✅ Все права ROLE_USER
- ✅ Создание/обновление/удаление отелей
- ✅ Создание/обновление/удаление номеров
- ✅ Создание/обновление/удаление пользователей
- ✅ Просмотр всех бронирований

### Защита эндпойнтов

| Эндпойнт | Доступ | Описание |
|----------|--------|----------|
| POST `/api/user/register` | PUBLIC | Открытая регистрация |
| POST `/api/user/auth` | PUBLIC | Открытая аутентификация |
| GET `/api/hotels` | USER, ADMIN | Просмотр отелей |
| POST `/api/hotels` | ADMIN | Создание отеля |
| POST `/api/bookings` | USER, ADMIN | Создание бронирования |
| POST `/api/user` | ADMIN | Создание пользователя |
| DELETE `/api/user/{id}` | ADMIN | Удаление пользователя |

### Безопасность паролей
- Пароли хэшируются с использованием **BCrypt**
- Минимальная сложность пароля не требуется (можно добавить валидацию)
- Пароли никогда не возвращаются в API ответах

### Межсервисное взаимодействие

#### Передача JWT между сервисами
1. Клиент отправляет запрос с JWT через Gateway
2. Gateway передает JWT токен в backend-сервисы
3. Каждый сервис проверяет JWT самостоятельно через `JwtAuthenticationFilter`
4. При вызове Hotel Service из Booking Service токен передается автоматически

#### Внутренние эндпойнты
Эндпойнты `/api/rooms/{id}/confirm-availability` и `/api/rooms/{id}/release` предназначены для межсервисного взаимодействия и также защищены JWT.

### Защита от атак

- ✅ **CSRF Protection:** Отключена для REST API (stateless)
- ✅ **CORS:** Настраивается в Gateway при необходимости
- ✅ **SQL Injection:** Защита через JPA/Hibernate
- ✅ **XSS:** JSON ответы автоматически экранируются
- ✅ **Brute Force:** Можно добавить rate limiting
- ✅ **Session Hijacking:** Используются stateless JWT токены

## ⚠️ Обработка ошибок

Система использует централизованную обработку ошибок через `@ControllerAdvice` в каждом сервисе.

### Формат ответа при ошибке

Все ошибки возвращаются в едином JSON формате:

```json
{
  "timestamp": "2026-01-07T20:15:30.123",
  "status": 401,
  "error": "Authentication failed",
  "message": "Invalid username or password"
}
```

#### Поля ответа:
- `timestamp` - Точное время возникновения ошибки (LocalDateTime)
- `status` - HTTP код ошибки (int)
- `error` - Краткое описание типа ошибки (String)
- `message` - Детальное сообщение об ошибке (String)

### HTTP коды ошибок

| Код | Название | Когда возникает | Пример |
|-----|----------|-----------------|--------|
| **400** | Bad Request | Некорректные параметры запроса | Неверный формат даты |
| **401** | Unauthorized | Неверные учетные данные | Неверный пароль |
| **403** | Forbidden | Недостаточно прав / Отсутствует токен | USER пытается создать отель |
| **404** | Not Found | Ресурс не найден | Запрос несуществующего бронирования |
| **409** | Conflict | Конфликт ресурсов | Дубликат username, двойное бронирование |
| **500** | Internal Server Error | Внутренняя ошибка сервера | Неожиданное исключение |

### Примеры ответов с ошибками

#### 401 Unauthorized - Неверный пароль
```bash
POST /api/user/auth
{
  "username": "admin",
  "password": "wrongpassword"
}

# Ответ:
HTTP/1.1 401 Unauthorized
{
  "timestamp": "2026-01-07T20:15:30.123",
  "status": 401,
  "error": "Authentication failed",
  "message": "Invalid username or password"
}
```

#### 403 Forbidden - Отсутствует токен
```bash
POST /api/bookings
# (без заголовка Authorization)

# Ответ:
HTTP/1.1 403 Forbidden
{
  "timestamp": "2026-01-07T20:16:15.456",
  "status": 403,
  "error": "Access denied",
  "message": "Access Denied"
}
```

#### 409 Conflict - Дубликат username
```bash
POST /api/user/register
{
  "username": "admin",
  "password": "test123"
}

# Ответ:
HTTP/1.1 409 Conflict
{
  "timestamp": "2026-01-07T20:16:45.789",
  "status": 409,
  "error": "User already exists",
  "message": "Username already exists"
}
```

#### 404 Not Found - Ресурс не найден
```bash
GET /api/bookings/999

# Ответ:
HTTP/1.1 404 Not Found
{
  "timestamp": "2026-01-07T20:17:20.321",
  "status": 404,
  "error": "Resource not found",
  "message": "Booking not found with id: 999"
}
```

#### 409 Conflict - Номер уже забронирован
```bash
POST /api/rooms/1/confirm-availability
{
  "requestId": "req-456",
  "bookingId": 2,
  "startDate": "2026-02-01",
  "endDate": "2026-02-05"
}

# Ответ:
HTTP/1.1 409 Conflict
{
  "timestamp": "2026-01-07T20:18:10.654",
  "status": 409,
  "error": "Room not available",
  "message": "Room is already booked for the requested dates"
}
```

#### 400 Bad Request - Валидация данных
```bash
POST /api/bookings
{
  "startDate": "2026-02-05",
  "endDate": "2026-02-01"  # endDate раньше startDate
}

# Ответ:
HTTP/1.1 400 Bad Request
{
  "timestamp": "2026-01-07T20:19:00.987",
  "status": 400,
  "error": "Validation failed",
  "message": "End date must be after start date"
}
```

### Обработка ошибок в клиентском коде

#### Java (WebClient)
```java
webClient.post()
    .uri("/api/bookings")
    .header("Authorization", "Bearer " + token)
    .bodyValue(bookingDto)
    .retrieve()
    .onStatus(HttpStatus::is4xxClientError, response -> 
        response.bodyToMono(ErrorResponse.class)
            .flatMap(error -> Mono.error(
                new BookingException(error.getMessage())
            ))
    )
    .bodyToMono(BookingDto.class)
    .block();
```

#### curl
```bash
# Сохранить полный ответ с ошибкой
curl -v -X POST http://localhost:8080/api/user/auth \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"wrong"}' 2>&1 | tee error.log

# Вывести только код ошибки
curl -s -o /dev/null -w "%{http_code}" \
  http://localhost:8080/api/bookings

# Получить JSON с ошибкой
curl -s -X POST http://localhost:8080/api/user/auth \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"wrong"}' | jq '.'
```

### Логирование ошибок

Все ошибки логируются на сервере:

```
2026-01-07 20:15:30.123 ERROR [...] GlobalExceptionHandler - Runtime exception occurred: Invalid username or password
java.lang.RuntimeException: Invalid username or password
    at ru.mephi.springfinal.booking.service.UserService.authenticate(UserService.java:63)
    ...
```

Уровни логирования:
- **ERROR** - RuntimeException, Exception (критичные ошибки)
- **WARN** - Ожидаемые ошибки (неверный пароль, не найден ресурс)
- **INFO** - Успешные операции
- **DEBUG** - Детальная информация для отладки

### Отладка ошибок

1. **Проверьте HTTP статус:**
   ```bash
   curl -v http://localhost:8080/api/bookings
   ```

2. **Посмотрите детали ошибки:**
   ```bash
   curl -s http://localhost:8080/api/bookings | jq '.'
   ```

3. **Проверьте логи сервиса:**
   ```bash
   make logs-booking
   make logs-hotel
   ```

4. **Включите DEBUG логирование:**
   В `application.yml`:
   ```yaml
   logging:
     level:
       ru.mephi.springfinal: DEBUG
   ```

### Best Practices

✅ **Всегда проверяйте HTTP статус** перед парсингом ответа
✅ **Показывайте пользователю `error.message`**, а не технические детали
✅ **Логируйте полный ответ об ошибке** для отладки
✅ **Используйте `timestamp`** для корреляции с логами сервера
✅ **Обрабатывайте разные коды ошибок** по-разному (401 → редирект на логин, 409 → показать конфликт)

## 🌐 Swagger UI и документация
- Hotel Service: http://localhost:8081/swagger-ui.html
- Booking Service: http://localhost:8082/swagger-ui.html
- API Gateway: http://localhost:8080/swagger-ui.html

## H2 Console

- Hotel Service: http://localhost:8081/h2-console (JDBC URL: `jdbc:h2:mem:hoteldb`)
- Booking Service: http://localhost:8082/h2-console (JDBC URL: `jdbc:h2:mem:bookingdb`)

## Особенности реализации

### Распределенные транзакции (Saga Pattern)

При создании бронирования используется паттерн Saga:

1. **PENDING** - Создается бронирование в статусе PENDING
2. **Confirmation** - Booking Service вызывает Hotel Service для подтверждения доступности
3. **CONFIRMED** - При успехе статус меняется на CONFIRMED
4. **COMPENSATED** - При ошибке выполняется компенсация (отмена резервации)

### Идемпотентность

Все операции создания бронирования идемпотентны благодаря использованию `requestId`:
- Повторный запрос с тем же `requestId` вернет существующее бронирование
- Предотвращает создание дубликатов при сетевых сбоях

### Retry и Timeout

- Вызовы между сервисами имеют настроенные timeout (5 секунд)
- Автоматические retry с exponential backoff (3 попытки)
- При исчерпании попыток выполняется компенсация

### Алгоритм балансировки загрузки

Hotel Service отслеживает количество бронирований каждого номера (`timesBooked`).
При автоподборе номера выбираются те, что имеют наименьшее количество бронирований.

### Cleanup Scheduler

- Hotel Service: очистка истекших резерваций каждую минуту
- Booking Service: очистка истекших PENDING бронирований каждую минуту

## Технологический стек

- Spring Boot 3.5.9
- Spring Cloud 2024.0.2
- Spring Data JPA
- Spring Security + JWT
- H2 Database (in-memory)
- Lombok
- MapStruct
- SpringDoc OpenAPI
- Netflix Eureka
- Spring Cloud Gateway
- WebFlux (для WebClient и Gateway)

## Логирование

Все сервисы поддерживают correlation ID через MDC:
- Заголовок `X-Correlation-Id` прослеживается через все сервисы
- Логи включают correlation ID для трассировки запросов

